...
XSU=({
  ...
  getMaxCol: function (workBook, sheetName) {
    const r = Object.keys(workBook.Sheets[sheetName])
      .map((c) => {
        const result = c.match(/^(?<col>[a-zA-Z]+)(?<row>[0-9]+)/);
        if (result?.groups?.col) {
          return result.groups.col;
        }
        return "";
      })
      .sort();
    const m = [...new Set(r)].reduce((pre, cur) => {
      if (cur.length > pre.length) {
        return cur;
      } else if (cur.length < pre.length) {
        return pre;
      } else {
        if (cur.localeCompare(pre)) {
          return cur;
        } else {
          return pre;
        }
    ...
    for (let i = 0; i < m.length; i++) {
      res = m[i].charCodeAt(0) - 64 + (res) * 26;
    }
    return res;
