$(document).ready(function () {
    ...
    $('#MB_btn').click(function () {
        ...
        //vm.BatchEqus = $.Enumerable.From(ksec.Common.GetModelSource("V_PRDBATCHDA.svc/web/GetList", { paras: [{ ColumnName: "BatchID", CompareStr: "=", ColumnValue1: $('#BatchID').val(), ColumnValue2: null }], GetWithChild: false })).ToArray();
    })

    $("#TQ_btn").click(function () {
        //$("#AnalysisPage").attr('src', "QuaAnalysis.html?BatchID=&Paras=");
        $("#AnalysisPage").attr('src', "QuaAnalysis.html?BatchID=" + encodeURIComponent($('#BatchID').val()) + "&Paras=" + encodeURIComponent(JSON.stringify(GetSelParas())));
    });
